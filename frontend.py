...
def create_account():
    ...
    if account_name is None:
        return
    current_file_output.append([constants.new_account_output_key,
                                account_number,
                                constants.empty_money_amount,
                                constants.empty_account_number,
                                account_name
...
def deposit():
    ...
    cents = get_valid_numeric_amount(
        constants.max_deposit_atm_once if user_status == constants.atm_state else constants.max_deposit_teller_once)
    if cents is None:
        return
    # Check if total deposits that day less than total
    if (user_status == constants.agent_state or total_less_than_daily_limit(constants.deposit_output_key,
                                                                            account_number,
...
def withdraw():
    ...
    # Check number less than max single transaction
    cents = get_valid_numeric_amount(
        constants.max_withdraw_atm_once if user_status == constants.atm_state else constants.max_withdraw_agent_once)
    if cents is None:
        return
    # Check if total withdrawl less than total limit
    if (user_status == constants.agent_state or total_less_than_daily_limit(constants.withdraw_output_key,
                                                                            account_number,
...
def transfer():
    ...
    to_account = get_valid_account_number(True, constants.to_account_prompt)
    # Check number less than max single transaction
    cents = get_valid_numeric_amount(
        constants.max_transfer_atm_once if user_status == constants.atm_state else constants.max_transfer_agent_once)
    if cents is None:
        return
    # Check if total withdrawl less than total limit
    if (user_status == constants.agent_state or total_less_than_daily_limit(constants.transfer_output_key,
                                                                            from_account,
